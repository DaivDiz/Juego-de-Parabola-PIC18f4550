...
void GLCD_DisplayChar(uint8_t ch)
{
 ...
 if(((GLCD.PageNum == 0x01) && ((GLCD.CursorPos+5)>=128)) || (ch=='\n'))
 {
...
void GLCD_DisplayNumber(uint8_t v_numericSystem_u8, uint32_t v_number_u32, uint8_t v_numOfDigitsToDisplay_u8)
{
    ...
    if(2u == v_numericSystem_u8)
    {
        while(v_numOfDigitsToDisplay_u8!=0)
        {
          ...
          i = (((v_number_u32)&(((uint32_t)1<<((v_numOfDigitsToDisplay_u8-1)))))!=0u);
          GLCD_DisplayChar(((i)+0x30));
    ...
    else if(v_number_u32==0)
    {
        ...
        for(i=0;((i<v_numOfDigitsToDisplay_u8) && (i<10u));i++)
            GLCD_DisplayChar('0');
    }
    else
    {
         ...
         while(i!=0)
        {
          ...
          GLCD_DisplayChar((((a[i-1])>0x09) ? ((a[i-1]) + 0x37): ((a[i-1]) + 0x30)));
          i--;
        }
    }
}
        ...
        if(ch == '%')
        {
           ...
           if((ch>=0x30) && (ch<=0x39))
            ...
            case 'd':
                ...
                GLCD_DisplayNumber(10u,v_num_s16,v_numOfDigitsToDisp_u8);
                break;
            ...
            case 'D':
                ...
                GLCD_DisplayNumber(10u,v_num_s32,v_numOfDigitsToDisp_u8);
                break;

            case 'u':
                v_num_u16 = (*(int *)__va_arg(*(int **)argp, (int)0));
                GLCD_DisplayNumber(10u,v_num_u16,v_numOfDigitsToDisp_u8);
                break;

            case 'U':
                v_num_u32 = (*(uint32_t *)__va_arg(*(uint32_t **)argp, (uint32_t)0));
                GLCD_DisplayNumber(10u,v_num_u32,v_numOfDigitsToDisp_u8);
                break;

            case 'x':
                v_num_u16 = (*(int *)__va_arg(*(int **)argp, (int)0));
                GLCD_DisplayNumber(16u,v_num_u16,v_numOfDigitsToDisp_u8);
                break;

            case 'X':
                v_num_u32 = (*(uint32_t *)__va_arg(*(uint32_t **)argp, (uint32_t)0));
                GLCD_DisplayNumber(16u,v_num_u32,v_numOfDigitsToDisp_u8);
                break;
            ...
            case 'b':
                ...
                GLCD_DisplayNumber(2u,v_num_u16,v_numOfDigitsToDisp_u8);
                break;
            ...
            case 'B':
                ...
                if(v_numOfDigitsToDisp_u8 == 0xffu)
                   v_numOfDigitsToDisp_u8 = 16;
                GLCD_DisplayNumber(2u,v_num_u32,v_numOfDigitsToDisp_u8);
                break;
...
void GLCD_SetCursor(uint8_t lineNumber,uint8_t CursorPosition)
{
 ...
 if((lineNumber <= 0x07u) && (CursorPosition <= 128))
 {
  pageNumber = CursorPosition / 64;
  CursorPosition = CursorPosition % 0x3f;
  if(pageNumber==0x00)
  {
   glcd_SelectPage0();
  }
  ...
  GLCD.PageNum = pageNumber;
  GLCD.LineNum=lineNumber | 0xB8;
  GLCD.CursorPos=CursorPosition |0x40;
...
void GLCD_VerticalGraph(uint8_t var_barGraphNumber_u8, uint8_t var_percentageValue_u8)
{
    ...
    if((var_barGraphNumber_u8 < 4) && (var_percentageValue_u8<=100))
    {
        ...
        for(i=1;i<8;i++)
        {
            ...
            {
                var_valueToDisplay_u8 = (uint8_t) (var_percentageValue_u8 - (8 * (uint8_t) (var_percentageValue_u8/8)));
...
{
 glcd_BusyCheck();
 PORTB = dat ^ GLCD.Invertdisplay;
 (((PORTD)) |= ((uint32_t)1<<((3))));
 (((PORTD)) &= ~((uint32_t)1<<((2))));
 (((PORTD)) |= ((uint32_t)1<<((4))));
    _delay((unsigned long)((0.5)*(16000000/16000000.0)));
