...
0x80, 0x80, 0x80, 0x80, 0x80,   // _        //0x40, 0x40, 0x40, 0x40, 0x40,   // _  -> Configuración Original
0x80, 0x00, 0x00, 0x00, 0x00,   // `        //0x00, 0x03, 0x05, 0x00, 0x00,   // `  ->Configuración Original
...
0x3F, 0x00, 0x00, 0x00, 0x00,   // |        //0x00, 0x00, 0xFF, 0x00, 0x00,   // | -> Configuración original
...
void GLCD_Init()
{
    ...
    GlcdCtrlBusDirnReg &= ~((1<<CS1)|(1<<CS2)|(1<<RW)|(1<<RS)|(1<<EN)); // Configure the Ctrl pins as output
...
void glcd_DataWrite(uint8_t dat)           //void glcd_DataWrite( uint8_t dat)
{
	glcd_BusyCheck();
...
void glcd_SelectPage0()
{
	GlcdSetBit(CS2);            //Estan invertidos los puertos CS1 y CS2
	GlcdClearBit(CS1);
}
